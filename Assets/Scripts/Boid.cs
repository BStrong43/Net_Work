...
public class Boid: MonoBehaviour
{
    ...
    //Required for dead reckoning, not sure where to update
    float time, lastPingTime, lerpDist;
    ...
    {
        float dt = (gameTime - time) / 1000f;//ms

        checkCollision(with);
        
        simulate(dt);
        if (gameTime - lastPingTime > 100)
        {
            lerpPosition(Mathf.Min(dt / lerpDist, 1f));
        }
        else
        {
            setPosition();
    ...
    public void lerpPosition(float t)
    {
        for (int i = 0; i < MAX_BUCKS; i++)
        {
            gameObjects[i].transform.position = Vector3.Lerp(gameObjects[i].transform.position, bucks[i].position.toVector3(), t);
        }
    }
